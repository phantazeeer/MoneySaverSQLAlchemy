...
async def add(record: Annotated[AddRecord, Form()], user_id: Annotated[int, Depends(get_jwt_payload)],
              service: Annotated[CEService, Depends(get_service)]) -> str:
    await service.add_record(user_id, **(record.model_dump()))
    return "OK"


@router.get('/{id}')
async def get(id: Annotated[int, Path()], user_id: Annotated[int, Depends(get_jwt_payload)],
              service: Annotated[CEService, Depends(get_service)]) -> Record | None:
    try:
...
@router.delete('/{id}')
async def delete(id: int, user_id: Annotated[int, Depends(get_jwt_payload)],
                 service: Annotated[CEService, Depends(get_service)]) -> str | None:
    try:
