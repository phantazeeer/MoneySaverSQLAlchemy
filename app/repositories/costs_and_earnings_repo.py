...
class CostsAndEarningsRepository(BasicRepository):
    ...
    async def add_one(
        ...
        record = await BasicRepository.add_one(
            self,
            user_id=user_id,
    ...
    async def update_record(
        ...
        await self.session.execute(
            update(self.model)
            .values(operation_type=operation_type, value=value, comment=comment, category_id=category_id)
    ...
    async def delete_by_id(self, id: int) -> None:
        ...
        await self.session.execute(
            update(User)
            .values(balance=User.balance - record.value * (-1) ** record.operation_type)
    ...
    async def get_list_by_date(self, start: datetime, end: datetime, user_id: int):
        record = await self.session.execute(
            select(CostsAndEarnings)
            .where(
                CostsAndEarnings.created_at >= start,
